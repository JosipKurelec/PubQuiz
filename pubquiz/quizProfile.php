...
$query = "SELECT COUNT(*) FROM teamonquiz where teamID = ? AND quizId = ?;";
$stmt = mysqli_stmt_init($db);

foreach ($idArray as $editionId) {
    if (mysqli_stmt_prepare($stmt, $query)) {
        mysqli_stmt_bind_param($stmt, 'ii', $_SESSION['id'], $editionId);
        mysqli_stmt_execute($stmt);
        mysqli_stmt_bind_result($stmt, $count);
        mysqli_stmt_fetch($stmt);
        mysqli_stmt_close($stmt);

        $TeamStatusArray[] = ($count > 0) ? 1 : 0;
    }
}

$query = "SELECT COUNT(*) FROM invites where teamID = ? AND EditionId = ?;";
$stmt = mysqli_stmt_init($db);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if($_SESSION['id'] != 0){
    $query = "INSERT INTO invites (teamId, editionId) VALUES (?, ?)";
    $stmt2=mysqli_stmt_init($db);
    if(mysqli_stmt_prepare($stmt2,$query)){
        mysqli_stmt_bind_param($stmt2,'ii', $_SESSION['id'], $_POST['editionOfQuizId']);
  ...
  <div class="row">
    ...
    <div class="col-lg-12 col-12 offset-lg-0 offset-0 setBox br0 row" >
        ...
        <table>
            ...
            <?php if(count($idArray)>0){for ($i = 0; $i < count($idArray); $i++) {
                ...
                echo '<tr style="border-bottom: 3px solid #cccccc;">
                    <td>
                        <div class="pd5 br5 row" style="background-color: ' . hexToRgba($Color) . '"><h4 class="nameofquiz">' . $NameOfEditionArray[$i] . '</h4><div class="col-sm-3 col-12"><b>Category</b><p>' . $ThemeOfQuizArray[$i] . '</p><b>Location</b><p>' . $LocationArray[$i] . '</p><b>Time of quiz</b><p>' . $DateOfQuizArray[$i] . '</br>' . $TimeOfQuizArray[$i] . '</p></div><div class="col-sm-3 col-12"><b>Elo</b><p>' . $EloArray[$i] . '</p><b>Awards</b><p>1st: ' . $rew[0] . '</p><p>2nd: ' . $rew[1] . '</p><p>3rd: ' . $rew[2] . '</p><b>Registration fee</b><p>' . $RegFeeArray[$i] . '€</p></div><div class="col-sm-3 col-12 offset-sm-3"><form method="post"><input type="hidden" name="editionOfQuizId" value="' . $idArray[$i] . '"><button class="tab-btn br5prc" type="submit" '; if($TeamStatusArray[$i] == 1 || $TeamStatus2Array[$i] == 1){echo 'disabled';} echo ' >Apply for quiz</button></form><b>' . $NoTeamsArray[$i] . '/' . $MaxTeamsArray[$i] . 'teams</b></div>
                        </div>
                    </td>
                </tr>';
                }
        ...
        <table>
            ...
            <?php if(count($idArrayp)>0){for ($i = 0; $i < count($idArrayp); $i++) {
